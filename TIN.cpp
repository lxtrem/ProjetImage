...
int main(int argc, char** argv) {
    if (argc <= 1) { // Il faut au moins une commande, une source et une destination
        cerr << "Pas assez d'arguments. Aide: ./TIN -h" << endl;
        return 0; 
    }
    char c;
    string source, cible;
    ...
    if (doubleseuillage)
        ecrirePGM( doubleSeuillage( lirePGM(source), 500, 80), cible);
    if (superpixel)
